...
#[test]
fn live_site_carries_graph_capabilities() {
    let site = live_sharepoint_site();
    let caps: Vec<_> = site
        .required_caps
        .iter()
    ...
    assert_eq!(site.site_id, SHAREPOINT_SITE_ID);
...
fn site_registers_as_export_site() {
    ...
    let record = register_sharepoint_site(&mut cx, true).unwrap();

    assert_eq!(record.kind, ExportKind::named(ExportKind::SITE));
    assert_eq!(record.symbol, site_symbol(SHAREPOINT_SITE_ID));
    assert!(matches!(
        record.state,
    ...
    assert!(
        cx.registry()
            .site_by_symbol(&site_symbol(SHAREPOINT_SITE_ID))
            .is_some()
    );
}
...
fn recipes_are_embedded() {
    ...
    assert!(
        cards
            .iter()
            .any(|card| card.id.ends_with("sharepoint-graph-read"))
    );
}
